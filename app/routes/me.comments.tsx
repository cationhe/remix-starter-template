...
export async function loader({ request, context }: LoaderFunctionArgs) {
	...
	const countRow = await queryOne<{ count: number | string }>(
		db,
		`SELECT COUNT(1) as count FROM comments c WHERE c.author_id = ? AND ${where}`,
		[me.id],
	);
	...
	return json<LoaderData>({ me, comments, page, pageSize, totalCount, totalPages, showDeleted });
}
...
export async function action({ request, context }: ActionFunctionArgs) {
	...
	if (ids.length === 0) {
		return json<ActionData>({ ok: false, error: "未选择要删除的评论" }, { status: 400 });
	}
	if (ids.length > 500) {
	...
	const owned = await queryAll<{ id: number }>(
		db,
		`SELECT id as id FROM comments WHERE author_id = ? AND deleted_at IS NULL AND id IN (${placeholders})`,
		[me.id, ...ids],
	);
	const ownedIds = owned.map((r) => r.id);
	if (ownedIds.length === 0) {
		return json<ActionData>({ ok: false, error: "无可删除的评论" }, { status: 400 });
	}
	const ownedPlaceholders = ownedIds.map(() => "?").join(",");
	const now = Date.now();
	await execute(
		db,
		`UPDATE comments SET deleted_at = ?, deleted_by = ? WHERE author_id = ? AND deleted_at IS NULL AND id IN (${ownedPlaceholders})`,
	...
	try {
		await execute(
			...
			[
				me.id,
				"me_comments_soft_deleted",
				ip,
				userAgent,
				JSON.stringify({ count: ownedIds.length, commentIds: ownedIds }),
				now,
			],
		);
	} catch {
...
export default function MeCommentsPage() {
	...
	const allOnPageSelected =
		!data.showDeleted && data.comments.length > 0 && data.comments.every((c) => selectedIds.has(c.id));
	...
	function toggleSelectAllOnPage() {
		setSelectedIds((prev) => {
			...
			if (allOnPageSelected) {
				for (const c of data.comments) next.delete(c.id);
			} else {
				for (const c of data.comments) next.add(c.id);
			}
			return next;
		});
	}
		...
		<div className="min-h-screen bg-gray-50 px-4 py-8 dark:bg-gray-900">
			<div className="mx-auto flex max-w-4xl flex-col gap-6">
				<header className="flex items-start justify-between gap-4">
					<div>
						<h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">我的评论管理</h1>
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						<Link
							to={data.showDeleted ? "/me/comments" : "/me/comments?showDeleted=1"}
							className="rounded border border-gray-300 px-3 py-1 text-sm text-gray-900 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-100 dark:hover:bg-gray-800"
						>
					...
					<table className="w-full table-auto text-left text-sm">
						<thead className="bg-gray-100 text-xs text-gray-600 dark:bg-gray-900/30 dark:text-gray-300">
							<tr>
								...
								<th className="px-4 py-3">所属帖子</th>
						...
						<tbody className="divide-y divide-gray-200 dark:divide-gray-700">
							{data.comments.map((c) => {
								const preview = String(c.content || "")
									.replace(/\s+/g, " ")
									.replace(/\u0000/g, "")
									.trim();
								const shown = preview.length > 80 ? `${preview.slice(0, 80)}…` : preview;
								const canLinkPost = !c.postDeletedAt && !data.showDeleted;
								return (
									<tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/20">
										...
										<td className="px-4 py-3">
											{canLinkPost ? (
												...
												<span className="text-gray-900 dark:text-gray-100">{c.postTitle}</span>
											)}
											<div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
												评论ID: {c.id}，帖子ID: {c.postId}
											</div>
										</td>
										<td className="px-4 py-3 text-gray-700 dark:text-gray-200">{shown || "(空)"}</td>
